...
class DogDAO @Inject()(@NamedDatabase("slick_test2") protected val dbConfigProvider: DatabaseConfigProvider) extends HasDatabaseConfigProvider[RelationalProfile] {
  import driver.api._

  private val Dogs = TableQuery[DogsTable]

  def getCreateSchema = Dogs.schema.create
  ...
  dbConfig.db.run(getCreateSchema)
}